module Averageable
  ...
  def average_in_all_scores(averageable)
    if self.number_of_votes == 1
      ...
      self.people_score          = (self.people_score * (self.number_of_votes - 1) + averageable.people_score) / 
                                      self.number_of_votes
      self.music_score           = (self.music_score * (self.number_of_votes - 1) + averageable.music_score) / 
                                      self.number_of_votes
      self.venue_score           = (self.venue_score * (self.number_of_votes - 1) + averageable.venue_score) / 
                                      self.number_of_votes
      self.atmosphere_score      = (self.atmosphere_score * (self.number_of_votes - 1) + 
                                      averageable.atmosphere_score) / self.number_of_votes
  ...
  def remove_averageable_from_average(average, self_number_of_votes)
    if average.number_of_votes == self_number_of_votes
      ...
      people_score     = average.people_score * average.number_of_votes
      music_score      = average.music_score * average.number_of_votes
      venue_score      = average.venue_score * average.number_of_votes
      atmosphere_score = average.atmosphere_score * average.number_of_votes
      average.number_of_votes -= self_number_of_votes
      average.people_score          = (people_score - self.people_score) / average.number_of_votes
      average.music_score           = (music_score - self.music_score) / average.number_of_votes
      average.venue_score           = (venue_score - self.venue_score) / average.number_of_votes
      average.atmosphere_score      = (atmosphere_score - self.atmosphere_score) / average.number_of_votes
